        ...
        if (this.state.queues && this.state.queues.length != 0){
            tmp = this.state.queues.map((q) => (
                <Menu.Item 
                    ...
                    <Grid style={chooseQueue === q.queueId ? style.widthNav2 : style.widthNav} verticalAlign="middle">
                        <Grid.Column width={5} style={chooseQueue === q.queueId ? style.queueNoNav2 : style.queueNoNav}>
                            {this.returnQueueId(q.queueId)}
                        </Grid.Column>
                        <Grid.Column width={11}>
                            <List>
                                <List.Content>
                                    <List.Header as="a" style={chooseQueue === q.queueId ? style.hnNoNav2 : style.hnNoNav}>
                                        {q.citizenId}
                                </List.Header><br />
